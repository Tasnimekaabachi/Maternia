...
try {
    ...
    $event = new Google\Service\Calendar\Event([
        'summary' => 'Test Meet Maternia Final V4',
    ...
    // Attendre plus longtemps et boucler pour voir si le lien arrive
    for ($i = 0; $i < 5; $i++) {
        sleep(2);
        ...
        if ($link) {
            ...
            if ($created->getConferenceData()) {
                $status = $created->getConferenceData()->getCreateRequest()->getStatus()->getStatusCode();
                echo "Status: " . $status . "\n";
            }
        }
    }
