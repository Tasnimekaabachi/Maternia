...
// Let's use primary first to see if it works there
...
try {
    ...
    echo "Event created ID: " . $created->getId() . "\n";
    ...
    for($i=0; $i<5; $i++) {
        ...
        if ($link) {
            ...
            if ($created->getConferenceData() && $created->getConferenceData()->getCreateRequest()) {
                echo "Status: " . $created->getConferenceData()->getCreateRequest()->getStatus()->getStatusCode() . "\n";
            }
        }
    }
